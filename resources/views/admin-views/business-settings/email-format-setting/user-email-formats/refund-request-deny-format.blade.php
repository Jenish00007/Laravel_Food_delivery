    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','refund_request_deny_mail_status_user')->first()?->value ??  '0' )
                    <div class="card-body">
                        ...
                        <span class="form-label-secondary text--primary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('Customers_will_receive_an_automated_mail_from_the_system_if_the_Admin_rejects_their_Refund_Request.') }}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                <input type="checkbox" class="status toggle-switch-input dynamic-checkbox"
                                       ...
                                       data-text-off="<p>{{translate('If_disabled,_Customers_will_not_receive_any_mail_for_Refund_Request_rejection.')}}</p>"
                        ...
                        <form action="{{route('admin.business-settings.email-status',['user','refund-request-deny',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\App\Models\EmailTemplate::where('type','user')->where('email_type', 'refund_request_deny')->first())
                @php($template= $template ?? $data?->email_template ?? 8)
                <form action="{{ route('admin.business-settings.email-setup', ['user','refund-request-deny']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->where('type','user')->where('email_type', 'refund_request_deny')->first())

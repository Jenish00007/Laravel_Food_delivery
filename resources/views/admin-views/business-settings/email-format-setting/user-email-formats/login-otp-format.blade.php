    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','login_otp_mail_status_user')->first()?->value  ?? '0')
                    <div class="card-body">
                        ...
                        <span class="form-label-secondary text--primary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('Customers_will_receive_an_OTP_every_time_they_log_in_to_their_account_via_the_Customer_App_or_Website.') }}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                ...
                                <input type="checkbox" class="status toggle-switch-input dynamic-checkbox"
                                       ...
                                       data-title-on="{{translate('Want_to_enable_User_login_mail?')}}"
                                       data-title-off="{{translate('Want_to_disable_User_login_mail?')}}"
                                       data-text-on="<p>{{translate('If_enabled,_customers_will_receive_a_login_OTP_email_every_time_they_log_in_to_their_account.')}}</p>"
                                       data-text-off="<p>{{translate('If_disabled,_customers_will_not_receive_any_OTP_email_during_Login.')}}</p>"
                        ...
                        <form action="{{route('admin.business-settings.email-status',['user','login-otp',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\App\Models\EmailTemplate::where('type','user')->where('email_type', 'login_otp')->first())
                @php($template= $template ?? $data?->email_template ?? 4)
                <form action="{{ route('admin.business-settings.email-setup', ['user','login-otp']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->where('type','user')->where('email_type', 'login_otp')->first())

    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','refund_order_mail_status_user')->first()?->value ??  '0' )
                    <div class="card-body">
                        ...
                        <span class="form-label-secondary text--primary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('Customers_will_get_an_automated_email_when_they_receive_a_refund_to_their_wallet_from_Admin_with_refund_details.') }}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                ...
                                <input type="checkbox" class="status toggle-switch-input dynamic-checkbox"
                                       ...
                                       data-title-on="{{translate('Want_to_enable_Refund_Order_mail?')}}"
                                       data-title-off="{{translate('Want_to_disable_Refund_Order_mail?')}}"
                                       data-text-on="<p>{{translate('If_enabled,_Customers_will_get_an_automated_email_when_they_receive_a_refund.')}}</p>"
                                       data-text-off="<p>{{translate('If_disabled,_Customers_will_not_receive_any_mail_on_Refund_Orders.')}}</p>"
                ...
                @php($data=\App\Models\EmailTemplate::where('type','user')->where('email_type', 'refund_order')->first())
                @php($template= $template ?? $data?->email_template ?? 9)
                <form action="{{ route('admin.business-settings.email-setup', ['user','refund-order']) }}" method="POST" enctype="multipart/form-data">
                    @csrf

    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','add_fund_mail_status_user')->first()?->value ?? '0')
                    ...
                    <div class="card-body">
                        <div class="maintenance-mode-toggle-bar d-flex flex-wrap justify-content-between border rounded align-items-center p-2">
                            <h5 class="text-capitalize m-0 text--primary pl-2">
                                {{translate('Send_Mail_On_‘Fund_Add’?')}}
                        <span class="form-label-secondary text--primary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('Customers_will_receive_an_automated_mail_from_the_system_when_Admin_add_fund_to_their_Wallet.') }}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                <input type="checkbox" class="status toggle-switch-input dynamic-checkbox"
                                        ...
                                        data-title-on="{{translate('Want_to_enable_add_fund_mail?')}}"
                                        data-title-off="{{translate('Want_to_disable_add_fund_mail?')}}"
                                        data-text-on="<p>{{translate('If_enabled,_customers_will_receive_an_email_when_Admin_adds_funds_to_their_wallet.')}}</p>"
                                        data-text-off="<p>{{translate('If_disabled,_Customers_will_not_receive_an_email_on_Added_Funds.')}}</p>"
                        ...
                        <form action="{{route('admin.business-settings.email-status',['user','add-fund',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\App\Models\EmailTemplate::where('type','user')->where('email_type', 'add_fund')->first())
                @php($template= $template ?? $data?->email_template ?? 6)
                <form action="{{ route('admin.business-settings.email-setup', ['user','add-fund']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->where('type','user')->where('email_type', 'add_fund')->first())

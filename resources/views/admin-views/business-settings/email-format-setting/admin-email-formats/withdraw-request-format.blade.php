    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','withdraw_request_mail_status_admin')->first()?->value ?? '0' )
                    <div class="card-body">
                        <div class="maintenance-mode-toggle-bar d-flex flex-wrap justify-content-between border rounded align-items-center p-2">
                            <h5 class="text-capitalize m-0 text--primary pl-2">
                                {{translate('Send Mail on Withdraw Request ?')}}
                        <span class="form-label-secondary text--primary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('If_a_store_requests_for_a_withdrawal,_admin_will_receive_an_automated_email.') }}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                <input type="checkbox" class="status toggle-switch-input dynamic-checkbox"
                                       ...
                                       data-title-on="{{translate('Want_to_enable_Withdraw_Request_mail?')}}"
                                       data-title-off="{{translate('Want_to_disable_Withdraw_Request_mail?')}}"
                                       data-text-on="<p>{{translate('If_enabled,_admin_will_receive_an_email_when_a_store_requests_a_withdrawal.')}}</p>"
                                       data-text-off="<p>{{translate('If_disabled,_admin_will_not_receive_an_email_when_a_store_requests_a_withdrawal.')}}</p>"
                        ...
                        <form action="{{route('admin.business-settings.email-status',['admin','withdraw-request',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\App\Models\EmailTemplate::where('type','admin')->where('email_type', 'withdraw_request')->first())
                @php($template= $template ?? $data?->email_template ?? 6)
                <form action="{{ route('admin.business-settings.email-setup', ['admin','withdraw-request']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->where('type','admin')->where('email_type', 'withdraw_request')->first())

                        ...
                        <span class="form-label-secondary text--primary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('If_a_store_requests_to_join_campaign_an_automated_email_will_be_sent_to_the_admin.') }}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                <input type="checkbox" class="status toggle-switch-input dynamic-checkbox"
                                        ...
                                        data-text-on="<p>{{translate('If_enabled,_the_admin_will_receive_a_mail_when_a_store_requests_to_join_a_campaign.')}}</p>"
                                        data-text-off="<p>{{translate('If_disabled,_the_admin_will_not_receive_mail_when_a_store_requests_to_join_a_campaign.')}}</p>"
                                        id="mail-status1" {{$mail_status == '1'?'checked':''}}>
                                <span class="toggle-switch-label text mb-0">
                                    <span class="toggle-switch-indicator"></span>
                                </span>
                            </label>
                        </div>
                        <form action="{{route('admin.business-settings.email-status',['admin','campaign-request',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\App\Models\EmailTemplate::where('type','admin')->where('email_type', 'campaign_request')->first())
                @php($template= $template ?? $data?->email_template ?? 1)
                <form action="{{ route('admin.business-settings.email-setup', ['admin','campaign-request']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->where('type','admin')->where('email_type', 'campaign_request')->first())

    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','deny_mail_status_dm')->first()?->value ?? '0')
                    ...
                    <div class="card-body">
                        ...
                        <span class="form-label-secondary text--primary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('If_Admin_rejects_a_Deliveryman’s_self-registration,_the_Deliveryman_will_get_an_automatic_rejection_mail.') }}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                <input type="checkbox" class="status toggle-switch-input dynamic-checkbox"
                                       ...
                                       data-text-off="<p>{{translate('If_disabled,_Delivery_Men_will_not_receive_any_email_upon_rejection_for_registration.')}}</p>"
                        ...
                        <form action="{{route('admin.business-settings.email-status',['dm','deny',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\App\Models\EmailTemplate::where('type','dm')->where('email_type', 'deny')->first())
                @php($template= $template ?? $data?->email_template ?? 5)
                <form action="{{ route('admin.business-settings.email-setup', ['dm','deny']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->where('type','dm')->where('email_type', 'deny')->first())

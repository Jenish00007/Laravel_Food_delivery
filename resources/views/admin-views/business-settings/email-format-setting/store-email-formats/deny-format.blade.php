    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','deny_mail_status_store')->first()?->value ?? '0')
                    <div class="card-body">
                        <div class="maintenance-mode-toggle-bar d-flex flex-wrap justify-content-between border rounded align-items-center p-2">
                            <h5 class="text-capitalize m-0 text--primary pl-2">
                                {{translate('Send_Mail_on_‘New_Store_Rejection’?')}}
                        <span class="form-label-secondary text--primary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('If_Admin_rejects_a_Store’s_self-registration,_the_store_will_get_an_automatic_disapproval_mail_from_the_system.') }}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                ...
                                data-title-on="{{translate('Want_to_enable_Store_deny_mail?')}}"
                                data-title-off="{{translate('Want_to_disable_Store_deny_mail?')}}"
                                data-text-on="<p>{{translate('If_enabled,_Users_will_receive_a_confirmation_email_when_the_Admin_rejects_their_registration_request.')}}</p>"
                                data-text-off="<p>{{translate('If_disabled,_Users_will_not_get_a_registration_rejection_mail.')}}</p>"
                        ...
                        <form action="{{route('admin.business-settings.email-status',['store','deny',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\App\Models\EmailTemplate::where('type','store')->where('email_type', 'deny')->first())
                @php($template= $template ?? $data?->email_template ?? 5)
                <form action="{{ route('admin.business-settings.email-setup', ['store','deny']) }}" method="POST" enctype="multipart/form-data">
                    @csrf

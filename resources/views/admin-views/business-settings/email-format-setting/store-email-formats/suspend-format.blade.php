    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','suspend_mail_status_store')->first())
                    ...
                    <div class="card-body">
                        <div class="maintainance-mode-toggle-bar d-flex flex-wrap justify-content-between border rounded align-items-center p-2">
                            <h5 class="text-capitalize m-0 text--primary pl-2">
                                {{translate('Send_Mail_on_Suspend_a_Store')}}
                                <span class="input-label-secondary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('If_Admin_suspends_a_store’s_,_the_store_will_get_a_suspend_mail_from_the_system.')}}">
                                    <img src="{{ asset('/public/assets/admin/img/info-circle.svg') }}" alt="{{ translate('messages.Send_Mail_on_Suspend_a_Store') }}">
                                </span>
                            </h5>
                            <label class="toggle-switch toggle-switch-sm">
                                ...
                                data-title-on="{{translate('Want_to_enable_Store_suspend_mail?')}}"
                                data-title-off="{{translate('Want_to_disable_Store_suspend_mail?')}}"
                                data-text-on="<p>{{translate('If_enabled,_Store_will_receive_an_email_for_account_suspension.')}}</p>"
                                data-text-off="<p>{{translate('If_disabled,_Store_will_not_receive_an_email_for_account_suspension.')}}</p>"
                        ...
                        <form action="{{route('admin.business-settings.email-status',['store','suspend',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\App\Models\EmailTemplate::where('type','store')->where('email_type', 'suspend')->first())
                @php($template= $template ?? $data?->email_template ?? 5)
                <form action="{{ route('admin.business-settings.email-setup', ['store','suspend']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->with('translations')->where('type','store')->where('email_type', 'suspend')->first())
                                        ...
                                        <div class="__bg-F8F9FC-card">
                                            ...
                                            <div class="form-group mb-0">
                                                @if ($language)
                                                       <div class="form-group lang_form default-form">
                                                            ...
                                                            <input type="text" maxlength="50" data-id="mail-copyright" name="copyright_text[]"  placeholder="{{ translate('Ex:_Copyright_2024_newone._All_right_reserved')}}" class="form-control" value="{{ $data?->getRawOriginal('copyright_text') }}">
                                                        </div>
                                                    @foreach(json_decode($language) as $lang)
                                                    <?php
                                           $translate = [];
                                           if($data && count($data['translations'])){
                                                        ...
                                                        <div class="form-group d-none lang_form" id="{{$lang}}-form3">
                                                            ...
                                                            <input type="text" maxlength="50" name="copyright_text[]"  placeholder="{{ translate('Ex:_Copyright_2024_newone._All_right_reserved')}}" class="form-control" value="{{ $translate[$lang]['copyright_text']??'' }}">
                                                        </div>
                                                    @endforeach
                                                @else
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" maxlength="50"  placeholder="{{ translate('Ex:_Copyright_2024_newone._All_right_reserved')}}"class="form-control" name="copyright_text[]" value="">
                                                </div>
                                                @endif
                                            </div>
                                        </div>

                    ...
                    <div class="card-body">
                        <div class="maintenance-mode-toggle-bar d-flex flex-wrap justify-content-between border rounded align-items-center p-2">
                            <h5 class="text-capitalize m-0 text--primary pl-2">
                                {{translate('Send_Mail_on_‘Campaign_Join_Rejection’?')}}
                        <span class="form-label-secondary text--primary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('If_a_Store_requests_to_join_a_Campaign_and_Admin_rejects_their_joining_request,_they_will_get_an_automated_Rejection_email_from_the_system.') }}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                <input type="checkbox" class="status toggle-switch-input dynamic-checkbox"
                                       ...
                                       data-text-on="<p>{{translate('If_enabled,_Stores_will_receive_an_email_on_campaign_joining_Rejection.')}}</p>"
                                       data-text-off="<p>{{translate('If_disabled,_Stores_will_not_receive_any_email_on_campaign_joining_Rejection.')}}</p>"
                        ...
                        <form action="{{route('admin.business-settings.email-status',['store','campaign-deny',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\App\Models\EmailTemplate::where('type','store')->where('email_type', 'campaign_deny')->first())
                @php($template= $template ?? $data?->email_template ?? 7)
                <form action="{{ route('admin.business-settings.email-setup', ['store','campaign-deny']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->where('type','store')->where('email_type', 'campaign_deny')->first())

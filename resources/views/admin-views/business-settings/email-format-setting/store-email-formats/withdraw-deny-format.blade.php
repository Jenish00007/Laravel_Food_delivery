    ...
    <div class="content container-fluid">
        ...
        <div class="tab-content">
            <div class="tab-pane fade show active">
                <div class="card mb-3">
                    @php($mail_status=\App\Models\BusinessSetting::where('key','withdraw_deny_mail_status_store')->first()?->value ?? '0')
                    ...
                    <div class="card-body">
                        ...
                        <span class="form-label-secondary text--primary" data-toggle="tooltip" data-placement="right" data-original-title="{{ translate('If_a_Store_requests_for_a_withdrawal_and_Admin_rejects_it,_the_Store_will_get_an_automated_Withdraw_Rejection_email_from_the_system.') }}">
                            ...
                            <label class="toggle-switch toggle-switch-sm">
                                <input type="checkbox" class="status toggle-switch-input dynamic-checkbox"
                                       ...
                                       data-text-off="<p>{{translate('If_disabled,_Stores_will_not_receive_any_Withdrawal_Rejection_mail.')}}</p>"
                        ...
                        <form action="{{route('admin.business-settings.email-status',['store','withdraw-deny',$mail_status == '1'?0:1])}}" method="get" id="mail-status_form">
                        </form>
                    </div>
                </div>
                @php($data=\App\Models\EmailTemplate::where('type','store')->where('email_type', 'withdraw_deny')->first())
                @php($template= $template ?? $data?->email_template ?? 6)
                <form action="{{ route('admin.business-settings.email-setup', ['store','withdraw-deny']) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="email-format-wrapper">
                                ...
                                <div class="right-content">
                                    <div class="d-flex flex-wrap justify-content-between __gap-15px mt-2 mb-5">
                                        @php($data=\App\Models\EmailTemplate::withoutGlobalScope('translate')->where('type','store')->where('email_type', 'withdraw_deny')->first())
